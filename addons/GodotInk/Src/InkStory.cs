...
public partial class InkStory : Resource
{
    ...
    private readonly Dictionary<string, Ink.Runtime.Story.VariableObserver> internalObservers =
        new();

    public static InkStory Create(string rawStory)
    {
        return new InkStory() { rawStory = rawStory };
    }
    ...
    private Ink.Runtime.Story.VariableObserver BuildObserver()
    {
        ...
        {
            if (!observers.TryGetValue(name, out var callables))
                return;
    ...
    /// variant if nothing is returned.
    /// </returns>
    public Variant EvaluateFunction(
        string functionName,
        out string textOutput,
        params Variant[] arguments
    )
    {
        object? result = runtimeStory.EvaluateFunction(
            functionName,
            out textOutput,
    ...
    public void BindExternalFunction(string funcName, Callable callable, bool lookaheadSafe = false)
    {
        ...
        object? trampoline(object?[] arguments) =>
            FromVariant(callable.Call(ToVariants(arguments)));
    }
    ...
    public void BindExternalFunction<T1, T2>(
        string funcName,
        Func<T1, T2, Variant> func,
        bool lookaheadSafe = false
    )
    {
        runtimeStory.BindExternalFunction(
            funcName,
            (Func<T1, T2, object?>)trampoline,
            lookaheadSafe
        );
    ...
    public void BindExternalFunction<T1, T2, T3>(
        string funcName,
        Func<T1, T2, T3, Variant> func,
        bool lookaheadSafe = false
    )
    {
        runtimeStory.BindExternalFunction(
            funcName,
            (Func<T1, T2, T3, object?>)trampoline,
            lookaheadSafe
        );
    ...
    public void BindExternalFunction<T1, T2, T3, T4>(
        string funcName,
        Func<T1, T2, T3, T4, Variant> func,
        bool lookaheadSafe = false
    )
    {
        runtimeStory.BindExternalFunction(
            funcName,
            (Func<T1, T2, T3, T4, object?>)trampoline,
            lookaheadSafe
        );
